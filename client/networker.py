...
class Networker(object):
    ...
    def recieve(self, game, client):
        ...
        while True:
            ...
            state = None
            if len(game.old_states) > 0 and game.current_state.time > packet.time:
                if packet.time < game.old_states[0].time:
                    # This packet is extremely old
                    # This shouldn't actually ever happen
                    print("Packet received that is not in game.old_states!\nPacket time: {0}\ngame.old_states: {1}".format(time, [i.time for i in game.old_states]))
                ...
                # That includes the old_states and current_state
                states = game.old_states[:]
                ...
                if not(0 <= d_time <= 1):
                    print("This should not happen!\nd_time:{0}\ntime:{1}\nstates[0].time:{2}\nstates[1].time:{3}\n\nold_states:{4}\n\n\n".format(d_time, time, states[0].time, states[1].time, [i.time for i in game.old_states]))
